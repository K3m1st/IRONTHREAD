...
This session was a full design, build, and first live deployment of a multi-agent offensive security framework for HackTheBox. Starting from a concept ("I want to test agents on HTB boxes before OSAI+") and ending with Sova, Oracle, WEBDIG, and ELLIOT running live on a real box. The architecture proved itself — and its gaps — in the same session.
...
| ORACLE | Strategic command — ingests all intel, CVE research, briefs operator, deploys agents | ✅ Complete v2 |
...
~/Desktop/HTB/boxes/{BOX_NAME}/
    ...
    ├── oracle/      ← CLAUDE.md + ORACLE_SYSTEM_PROMPT.md
...
### 4. Sova → Oracle as Primary Focus
Mid-session decision: de-prioritize specialist build-out, focus on Sova and Oracle as the command layer. The specialists are infantry — Sova and Oracle are the architecture that makes the operation coherent.

### 5. Oracle as Strategic Advisor
Oracle never self-authorizes. It briefs, recommends a single move, and waits for operator confirmation. Executive summary first, full detail below. CVE research is complete before anything surfaces to the operator.
...
- Explicit scope boundary from Oracle's deployment order
- Hard stop at 3 failed attempts on a single path
- `[NEW SURFACE]` status code for out-of-scope discoveries
- Return to Oracle loop — new surface triggers specialist deployment, not ELLIOT improvisation

### 7. Web Search as Operational Intelligence
Models operating purely on training data are like operators who haven't Googled since their training cutoff. Web search activation points added to both Oracle and ELLIOT:
- Oracle searches on every confirmed version number, every CVE research phase
...
The mechanism that solves the operator-as-orchestrator problem. Oracle writes a structured handoff file before deploying ELLIOT. ELLIOT reads it to confirm scope, authorization, and context files. If `elliot_authorized` is not `true`, ELLIOT does not deploy. This is the precursor to full MCP state management.
...
- Oracle built a high-quality attack surface document with ranked attack paths
- The overall intelligence picture was more than sufficient for exploitation

### What Failed
- ELLIOT deployed before Oracle processed WEBDIG's findings — operation.md showed both as PENDING when ELLIOT was invoked
- Without a validated scope from Oracle, ELLIOT improvised — started the right path (portal auth with gitbot creds) but had no stop conditions, no scope boundary, and no structured handoff to fall back on
- ~300k tokens burned on rogue exploration instead of scoped execution
- Root cause: no enforced sequencing between WEBDIG → Oracle re-evaluation → ELLIOT deployment

### Lessons Applied
- handoff.json schema designed to enforce sequencing
- ELLIOT v2 scope enforcement and stop conditions added
- Oracle v2 web search protocol added
...
The biggest outstanding problem: the operator is still manually managing the pipeline. Knowing which agent to invoke next, verifying operation.md reflects reality, ensuring Oracle processed specialist output before ELLIOT deploys — all of this mental overhead is on the operator.
...
- [ ] Add handoff.json write step to Oracle CLAUDE.md — Oracle writes it before every ELLIOT deployment
...
- Enforced sequencing — Oracle cannot mark ELLIOT authorized until specialist findings are ingested
...
5. Test full Sova → Oracle → Specialist → ELLIOT loop via MCP
...
The Conductor reads MCP state, determines current operation phase, tells operator which agent to invoke next and with what context, and blocks invalid transitions. ELLIOT cannot deploy if Oracle re-evaluation is pending. Specialists cannot deploy if Sova is not marked COMPLETE.
...
- Should the Oracle re-evaluation cycle be mandatory before any ELLIOT session, or only when new specialist findings exist?
...
~/Desktop/HTB/adversary-agents/
    ...
    └── templates/
        ...
        ├── oracle/
        │   ├── CLAUDE.md                  (v2)
        │   └── ORACLE_SYSTEM_PROMPT.md   (v2 — web search added)
        └── elliot/
            ├── CLAUDE.md                  (v2)
...
ORACLE (strategy), WEBDIG (web specialist), ELLIOT (exploit specialist).
